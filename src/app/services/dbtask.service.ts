...
export class DBTaskService {
  ...
  crearTabla() {
    this.db
      .executeSql(
        'create table IF NOT EXISTS books(name VARCHAR(32),price NUMBER)'
      )
      .then((result) => {
        alert('tabla creada');
      })
    ...
    this.db
      ...
      .then((result) => {
        for (let index = 0; index < result.rows.length; index++) {
          this.bookData.push({
            book_name: result.rows.item(index).name,
            book_price: result.rows.item(index).price,
          });
        }
      })
  ...
  selectByCodicion() {
    this.bookData = [];
    this.db
      .executeSql('select * from books where name=?', [this.book_name])
      .then((result) => {
        for (let index = 0; index < result.rows.length; index++) {
          this.bookData.push({
            book_name: result.rows.item(index).name,
  ...
  actualizar() {
    ...
    this.db
      .executeSql('update books set price=? where name=?', [precio, nombre])
      .then((result) => {
  ...
  eliminar() {
    this.bookData = [];
    this.db
      .executeSql('delete from books where name=?', [this.book_name])
      .then((result) => {
        alert(JSON.stringify(result));
      })
      .catch((e) => alert(JSON.stringify(e)));
